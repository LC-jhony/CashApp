...
## Project Overview
Laravel 12 loan management app with Filament 5 admin panel, Vite + Tailwind CSS v4. Calculates amortization schedules using French, German, and American methods.
...
| Testing | Pest PHP 4 |
| Database | SQLite (testing), MySQL/PostgreSQL (prod) |
...
### Setup
```bash
composer setup                    # Full setup (deps + assets + migrate)
...
./vendor/bin/pest --filter="test_name"          # Via pest directly
...
php artisan config:clear          # Clear caches after config changes
```
...
### Import Order
Organize imports: Core Laravel → Packages → App Models → App Traits
...
```php
TextInput::make('amount')->required()->numeric()->live()
...
- Run single: `php artisan test --compact --filter=testName`
- Use specific assertions: `assertSuccessful()`, `assertNotFound()`, `assertForbidden()` (not `assertStatus()`)
...
| Trait | Description |
|-------|-------------|
| `TraitFrances` | Constant payment |
| `TraitAleman` | Declining payment |
| `TraitAmericano` | Interest-only |

Each provides: `PayM()` (payment amount), `PlanMensual()` (schedule) for Mensual/Bimestral/Trimestral/Semestral/Anual frequencies.
...
- Configure via CSS `@theme` directive (no `tailwind.config.js`)
- Use `gap-*` for sibling spacing (not margins)
- Deprecated replacements: `bg-opacity-*` → `bg-black/*`, `flex-shrink-*` → `shrink-*`
...
- Clear config cache after config changes: `php artisan config:clear`
