...
## Project Overview
This is a Laravel 12 application with Filament 5 admin panel, using Vite + Tailwind CSS v4 for frontend. It manages loans with multiple amortization methods (French, German, American).
...
- **Testing:** Pest PHP
- **Database:** SQLite (default for testing), MySQL/PostgreSQL for production
...
# Run tests directly with artisan
php artisan test

# Run specific test file
php artisan test tests/Unit/ExampleTest.php

# Run tests matching a filter (single test)
...
# Run Pest tests directly with filter
...
# Run Pint with check (no auto-fix)
./vendor/bin/pint --test

# Clear config cache (often needed after changes)
...
|---------|------------|---------|
| Models | PascalCase | `Loan`, `Payment`, `User` |
| Controllers | PascalCase | `LoanController` |
| Traits | PascalCase with Trait suffix | `TraitFrances`, `TraitAleman` |
| Methods | camelCase | `calculateAmortization()` |
| Variables | camelCase | `$amount`, `$rate` |
| Database columns | snake_case | `user_id`, `created_at` |
| Filament Resources | PascalCase + Resource | `LoanResource`, `UserResource` |
| Filament Schemas | PascalCase + Form/Infolist | `LoanForm`, `UserInfolist` |
| Filament Tables | PascalCase + Table | `LoansTable`, `UsersTable` |
| Filament Pages | PascalCase | `ListLoans`, `CreateLoan`, `EditLoan` |

### Import Organization
Group imports in this order (use IDE auto-organize):
...
Follow the existing pattern for Filament resources:
```
app/Filament/Resources/[Name]/
├── [Name]Resource.php      # Main resource class
├── Schemas/
│   ├── [Name]Form.php      # Form configuration
│   └── [Name]Infolist.php # Display configuration
├── Tables/
│   └── [Name]Table.php     # Table configuration
└── Pages/
    ├── List[Name].php
    ├── Create[Name].php
    ├── Edit[Name].php
...
- Log errors with `\Log::error('message: ' . $e->getMessage())`
- Return early with defaults when validation fails:
```php
if (!$amount || !$rate) {
    $set('plans', []);
    return;
...
- Define `$casts` for date/JSON columns when needed

### Template Conventions
- Blade templates: snake_case files in `resources/views/`
- Use Filament components for admin panel
...
- Group Feature tests in `tests/Feature/`
- Group Unit tests in `tests/Unit/`

Example:
```php
test('loan calculation returns correct amount', function () {
    $loan = new Loan(['amount' => 10000, 'rate_id' => 1]);
    expect($loan->amount)->toBe(10000);
});
```

### Test Database
- Uses SQLite in-memory for testing (see `phpunit.xml`)
- Configure test environment variables in `phpunit.xml`

---

## Common Patterns

### Traits for Amortization
The app uses traits for different amortization methods:
- `TraitFrances` - French (constant payment)
- `TraitAleman` - German (declining payment)
- `TraitAmericano` - American (interest-only until end)

Each trait provides:
- `PayM()` - Calculate payment amount
- `PlanMensual()` - Generate payment schedule
    ...
    return $schema
        ->components([
            Fieldset::make('Section Name')
                ->schema([/* components */]),
            // Use live() and afterStateUpdated() for reactive forms
        ]);
}
```
...
```bash
php artisan make:model Loan -m          # Model + migration
php artisan make:filament-resource Loan  # Filament resource
php artisan make:mail WelcomeMail        # Mailables
